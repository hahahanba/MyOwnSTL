...
#undef min
    ...
    public:
        // 构造、复制、移动、析构函数
//        noexcept操作符执行编译时检查，如果表达式声明为不抛出任何异常，则返回true。
//        它可以在函数模板的noexcept说明符中使用，以声明函数将对某些类型抛出异常，但不会对其他类型抛出异常。
        vector() noexcept
        {try_init();}
        ...
        ~vector()
        {
            destroy_and_recover(begin__, end_, cap_ - begin_);
    ...
    private:
        ...
        void fill_init(size_typen, const value_type& value);
    ...
    template<class T>
    template<class... Args>
    void vector<T>::reallocate_emplace(vector::iterator pos, Args&& ...args)
    {
    ...
    // 重新分配空间并在pos处插入元素
    template<class T>
    void vector<T>::reallocate_insert(vector::iterator pos, const value_type &value)
    {
        const auto new_size = get_new_cap(1);
        auto new_begin = data_allocator::allocate(new_size);
        auto new_end = new_begin;
    ...
    template<class T>
    typename vector<T>::iterator
    vector<T>::fill_insert(vector::iterator pos, vector::size_type n, const value_type &value)
    {
        if (n == 0)
            return pos;
    ...
    template<class T>
    template<class IIter>
    void vector<T>::copy_insert(vector::iterator pos, IIter first, IIter last)
    {
        if (first == last)
            return;
