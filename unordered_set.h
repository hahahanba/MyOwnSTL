//
// Created by gy gao on 1/13/23.
//

#ifndef MYOWNSTL_UNORDERED_SET_H
#define MYOWNSTL_UNORDERED_SET_H
...
//
// 异常保证：
// myownstl::unordered_set<Key> / myownstl::unordered_multiset<Key> 满足基本异常保证，对以下等函数做强异常安全保证：
//   * emplace
//   * emplace_hint
//   * insert
...
namespace myownstl
{
    ...
    // 参数一代表键值类型，参数二代表哈希函数，缺省使用 myownstl::hash，
    // 参数三代表键值比较方式，缺省使用 myownstl::equal_to
    template <class Key, class Hash = myownstl::hash<Key>, class KeyEqual = myownstl::equal_to<Key>>
    class unordered_set
    {
    private:
        // 使用 hashtable 作为底层机制
...
#endif //MYOWNSTL_UNORDERED_SET_H
